...
1. Read the ticket thread with search_tickets and extract every requirement stated: item
   type, model or spec hints, quantity, who it's for, budget mentions, deadline drivers
   (start date, failed hardware, project milestone).
...
3. If model selection is needed, use web_search to check current-generation equivalents and
