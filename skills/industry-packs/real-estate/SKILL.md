...
2. Context: search_tickets for this brokerage + system history, and search_itglue / search_hudu for the stack: tenant, transaction platform, MLS association(s), lockbox vendor, BYOD policy and support-scope line, broker's designated approver. Docs tools vary per tenant — if absent, say what you could NOT verify; a brokerage with no BEC-response contact or BYOD policy documented is a flag worth raising.
