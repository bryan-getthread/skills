...
1. Context: search_tickets for this firm's history with the named product (stale-lock clears and plotter fixes repeat), and search_itglue / search_hudu for per-project versions where documented, the license model and server details, central-model locations, backup scope, and the plotter fleet. These docs tools vary per tenant — if absent, say what you could NOT verify; undocumented license servers or central-model locations are flags worth raising.
