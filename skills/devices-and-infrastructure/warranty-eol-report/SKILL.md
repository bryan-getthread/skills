...
Surfaces the lifecycle risk hiding in a client's fleet: OS versions past or approaching end of support, hardware old enough to be a reliability risk, and warranty expirations where any connected source actually records them.

## When to use

- "What's end-of-life or out of warranty at <client>?"
- QBR or budget-season prep: lifecycle risks to put in front of the client.
- After an OS vendor announces an end-of-support date: "who's exposed?"
...
6. Offer a plain-text note via `add_ticket_note`, and note that a client-facing version should go through the Client-Facing Device Report skill instead of this internal one.

## Guardrails

- Never fabricate warranty status or support dates — "unknown" with a lookup path beats a confident guess that ends up in a client budget meeting.
- Every hardware-age figure carries its evidence signal; ages inferred from first-seen dates are floors, not facts.
- Offline/stale devices get flagged as possibly retired rather than inflating the risk counts.
- Read-only skill; the output is a report, not a change plan.
- Result-cap honesty on every fleet listing.
