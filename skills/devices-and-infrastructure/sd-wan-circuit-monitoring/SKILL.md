...
1. Establish the site's circuit topology: how many circuits, primary vs secondary, carrier and account per circuit, and the SD-WAN/edge platform. Pull documented circuit inventory (search_itglue — cross-reference circuit-inventory) and the edge device from the RMM (search_ninjaone_devices).
...
6. Output a status summary per circuit plus the failover verdict and, if needed, the ready-to-send ISP escalation package. Post as a plain-text note (add_ticket_note, no markdown/emojis) or create_ticket. For a broader single-circuit outage, hand off to isp-outage-tracking.
