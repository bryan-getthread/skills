...
6. Output the per-site inventory table plus a drift/risk list with recommended actions (document X, add monitoring for Y, verify Z on site). Offer to open a documentation-update ticket (create_ticket) — updating the documentation platform itself is a handoff — and post a plain-text summary note (add_ticket_note, no markdown/emojis).
