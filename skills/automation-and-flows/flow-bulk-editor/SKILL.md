...
1. List all flows with list_flows. Identify the target set from the member's criteria
   (client, name pattern, current state) and get_flow for detail where needed to confirm
   each is the right one. Never act on a vague "all the X flows" without showing the
   resolved list first — act only on flows list_flows/get_flow actually return.
