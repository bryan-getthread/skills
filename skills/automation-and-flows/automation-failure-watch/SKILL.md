...
someone fixes the flow. (A Flow cannot natively send a Teams message; it calls Run Skill and
you use the Zapier action.)
...
1. Read the triggering note with search_tickets. Check it against the desk's configured
...
2. Post a plain-text internal note via add_ticket_note flagging the failure: which signature
