...
convenience. (A Flow cannot natively send an approval; it calls Run Skill and you use
send_approval.)

Your entire reply is posted verbatim as the internal note — plain text, no narration, no
markdown, no questions. Exactly two outcomes: (a) unambiguous approver -> send_approval +
audit note; (b) anything else -> no send, note-and-stop text stating why and what a human
must decide.

1. Confirm the ticket is (still) in the change-approval status; if it moved, stop.

2. Resolve the approver: search_contacts for this ticket's client filtered to the desk's
...
3. Unresolved approver (zero matches, ambiguous matches, inactive contact) -> note-and-stop.
   Post via add_ticket_note: "Change approval NOT sent: no unambiguous Change Approver
...
6. Post the audit note: "[change-approval] sent to <contact> (Change Approver) at <time>." —
