...
intent as part of the investigation. Flow/intent tools are admin-only; without them, ask
the member to paste the flow's configuration and debug from that.

1. Fetch both sides: get_flow (or get_intent) for the configuration, search_tickets for
   the ticket's actual attributes at the time of the event.
...
3. Walk every filter in a pass/fail table: filter, expected value, ticket's actual value,
   verdict. Use list_flow_filter_attribute_values to catch value-string mismatches ("High"
   vs "P2 - High"). Attribute values may lag after a recent update elsewhere — re-fetch the
   ticket before trusting a surprising value.

4. Check ordering and competition: list_flows for other flows on the same trigger — an
...
6. Output: the single named root cause (or ranked candidates if genuinely ambiguous), the
   ...
   with Thread support — do not invent an explanation. Apply the fix via update_flow /
   update_intent only after explicit confirmation.
```
