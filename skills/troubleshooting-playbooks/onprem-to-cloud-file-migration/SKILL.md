...
Scope, tool, and target design first. Use search_itglue / search_hudu / search_knowledge_base for the migration: the source (server, shares, total size/item count, deepest paths), the tool (SharePoint Migration Tool, Migration Manager, Mover, or a third-party like ShareGate/Metalogix), the target design (which shares → which SharePoint sites/libraries vs OneDrive personal), and the intended permission model on the target. A migration without a target information-architecture plan is the real problem behind most permission chaos — note if one exists. Confirm licensing/storage quota headroom on the target. IT Glue/Hudu coverage varies per tenant; if absent, fall back to search_knowledge_base and note what you couldn't check.

History first. Use search_tickets for this client + migration/SharePoint: earlier waves of the same migration (their error patterns repeat), a prior permission complaint, or a KFM/sync rollout. Reuse what a previous wave learned rather than rediscovering the same path-length wall.
