...
A tech is twenty minutes into a ticket and stuck. This skill runs the full research sweep in one pass — has anyone solved this before, is it documented, and what does the device actually look like right now — and returns a brief where every claim carries its source. new-ticket-first-touch does this at intake (plus classification and the acknowledgment draft); this skill is for a ticket already in flight, where the question is purely "what do we know that helps?"

## When to use

- "Any similar tickets with a resolution?" / "has this user or client had this before, and what fixed it?"
- A tech mid-troubleshoot wants everything known about this issue, this user, or this device in one place.
- Before escalating: gather what has already been tried desk-wide so L2/L3 does not repeat it.
...
7. Deliver in chat; post to the ticket as a plain-text note via `add_ticket_note` only if the tech asks for it on the record.
...
- Live RMM state is current-state only — do not reboot, modify services, or take any device action from this skill; remediation is the tech's call on the ticket.
- Degradation: no RMM connected → skip the device section and say so; no IT Glue/Hudu → note the docs gap; the brief still ships with whatever rings did return.
