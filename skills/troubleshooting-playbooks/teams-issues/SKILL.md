...
1. History first. Use search_tickets for this user and this client. Multiple users at once → tenant policy change or Microsoft service incident — check service health before touching endpoints.

2. Docs second. Use search_itglue / search_hudu / search_knowledge_base for the client's Teams standard: meeting policies, guest access stance, approved devices/headsets, VDI or shared-workstation quirks. IT Glue/Hudu coverage varies per tenant — note what you could not check.
...
5. Branch:
   ...
   - Audio/video device path — check Teams' own device settings first (wrong device selected after docking/undocking is the single most common cause), then OS-level privacy permissions (mic/camera allowed for Teams), then driver/firmware for the headset/camera (verify current versions with web_search on the vendor). One change at a time. Escalate when audio issues correlate with network (robotic audio, drops) → that's a call-quality issue; check the Wi-Fi/VPN path (media over full-tunnel VPN is a known killer — check the documented split-tunnel intent) and pair with teams-call-quality.
...
6. Verify and note. Reproduce the original failure path (join a real test meeting, confirm both directions of audio). Post a plain-text note via add_ticket_note: symptom, isolation result (web vs desktop), branch, action, verification.
