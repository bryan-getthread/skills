...
1. History first. Use search_tickets for this client + the failing app + certificate symptoms. A cluster of odd failures starting on one date usually marks the day inspection was enabled or its policy changed; earlier tickets may already list the approved bypass process.

2. Docs second. Use search_itglue / search_hudu / search_knowledge_base for the inspection landscape: which product inspects (perimeter firewall, cloud proxy/SSE agent, endpoint agent), whether the inspection root CA is deployed to trust stores and how, the existing bypass list, and who owns bypass decisions. IT Glue/Hudu coverage varies per tenant — fall back to search_knowledge_base and say what you could not check, including if the bypass process itself is undocumented (flag that as its own gap).
...
6. Close the loop. Re-test the failing app from the affected network after the bypass or trust fix lands, and confirm the certificate chain now looks as expected (vendor CA on bypassed endpoints, inspection CA elsewhere). Post a plain-text note via add_ticket_note: evidence (issuer observed on/off network), branch, vendor documentation cited, what was requested vs applied and who approved, verification result.
