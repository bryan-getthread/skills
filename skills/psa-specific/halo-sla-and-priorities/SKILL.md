...
1. Re-fetch the ticket with search_tickets at full detail — priority, status, and SLA state may
   have moved Halo-side since your last read.

2. Read the pair: which SLA applies (from the client/contract) and which priority is set
   (list_ticket_priorities for the tenant's actual list). The deadlines are the intersection;
   never quote a target from priority alone without knowing the client's SLA, since Halo targets
   differ per SLA.
...
4. Check pause state: hold-type statuses (waiting on customer/vendor/etc., per the desk's config)
   pause the SLA clock; verify the actual status against list_ticket_statuses rather than trusting
   a "waiting" phrase in a note. Only the status pauses the clock — a note does not.
...
5. When changing priority, treat it as a deadline change: state old → new priority and the
   ...
   multi-user impact force high priority). Apply with update_ticket after confirmation; note the
   reason via plain-text add_ticket_note.
...
Always: re-fetch before any deadline claim or priority change; a Halo-side response may have
