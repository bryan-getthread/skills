...
**When to use:** An "agent offline / sensor not checked in / endpoint unhealthy" alert fires from any endpoint-protection product; or a tech asks "is <device> actually unprotected or just powered off?" Fires on the alert ticket event, so it runs attended or as a Flow.
...
around unprotected TIME, not agent plumbing. Post a plain-text note only; change nothing else.
...
2. Dedupe/recurrence with search_tickets: same device + agent-offline in 30 days. A device
   whose agent repeatedly drops and returns is a broken-agent pattern (conflicting software,
   failed updates, tamper) — that is one reinstall/repair ticket, not a series of one-offs.
   If the search may have hit a result cap, say so.
3. The pivotal check — is the DEVICE online? Pull get_ninjaone_device (resolve via
...
4. For online-but-agent-dead: gather evidence via get_ninjaone_device_activities around the
   last check-in (agent update, app install, user action, reboot the service never survived)
   and list_ninjaone_alerts for related service-stopped alerts.
5. Classify: self-healed (console shows the agent has since checked in AND device state is
   ...
   N hours"), evidence, and the get_ninjaone_device_link deep link; needs-client (personal/BYOD
   or out-of-scope machine per docs) → route to account owner; noise (device decommissioned but
   never removed from console) → route as hygiene, never auto-close.
6. Post via add_ticket_note: verdict, device state vs agent state, unprotected duration,
