...
1. Get the real numbers first — no advice from vibes. Use search_tickets for context, then have the tech pull (PowerShell labeled: verify against current module versions):
   ...
   (search_knowledge_base or search_itglue for any documented client-specific mailbox notes — skip gracefully if IT Glue isn't connected.)
...
3. Pick the path, in cost order, and present it with numbers:
   ...
   Present the recommendation and the cost implication to the client contact for anything that touches licensing (send_approval).
...
6. Verify via evidence after the fix: fresh `Get-MailboxStatistics` showing size below quota, user confirms send works. Document what/why/when/rollback — post a plain-text note (add_ticket_note): sizes before/after, folder culprits, which path was taken and why, approvals, and what to do when it fills again (usually: the archive/upgrade this ticket deferred). Log time (log_time_entry).
