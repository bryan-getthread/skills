...
6. Approval and documentation. send_approval to the client authority for the migration plan — or for the stay recommendation, if the ticket asked to migrate and the answer is no (pushback is a deliverable). Pull the tenant's documented posture/licensing from IT Glue or Hudu if connected; degrade gracefully if not. Post a plain-text note (add_ticket_note): decision, rationale, licensing facts, sequence executed with dates, approver, and rollback (re-enable security defaults if the CA set must be torn down — crude but instant protection). This stay/migrate decision is the tenant's foundational access-control decision and must be auditable (see tenant-onboarding-checklist). Schedule the first periodic CA review (schedule_ticket) as part of migration completion.
