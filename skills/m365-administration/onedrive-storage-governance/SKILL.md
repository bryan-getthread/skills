...
5. Approval: quota changes, retention, sync restrictions, and sharing all affect users tenant-wide. Get client sign-off (send_approval) with each setting's before/after value stated.
...
7. Verify with evidence: quota reflects the change; a test share behaves at the set level; sync from a non-allowed device is blocked; retention shows applied. Document what/why/when/rollback — post a plain-text note (add_ticket_note): each setting's before/after, retention window and auto-access target, sync-scope rule, sharing level, approver, date, and rollback (restore prior values captured before the change). Log time (log_time_entry).
