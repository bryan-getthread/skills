...
1. Gather the decision inputs (search_tickets for context): does it send only to internal recipients or external too? What From address? Daily volume? Can the device do modern TLS and authentication, or is it a legacy appliance? Does it have a static public IP? (search_itglue / search_hudu for the device's documented specs; skip gracefully if neither is connected.)
...
4. Approval: new sending paths on the client's domain are spoofing surface — client sign-off (send_approval) on the method, sending address, and scope.
...
6. Verify via evidence: test message from the device to an internal and (if in scope) external recipient, delivered with headers showing the intended path; a message trace (mail-trace-investigation) confirming it. Document what/why/when/rollback in a plain-text note (add_ticket_note): device/app, method chosen and why, sending address, connector name and its exact IP/certificate scope, SPF change if any, approver, date, and rollback (disable connector <name>; revert SPF). Log time (log_time_entry).
