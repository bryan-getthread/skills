...
1. Decompose the ask into Purview terms and confirm each with the requester (search_tickets for context):
...
5. Approval gate: retention changes require the client's documented compliance/legal authority — not just "someone in IT" (send_approval). For anything with a delete action, the approval text must restate the scope and the destruction consequence.
...
8. Document what/why/when/rollback — post a plain-text note (add_ticket_note): policy name, exact scope and locations, retain vs delete behavior and duration, interaction with existing holds found in step 2, approver, application date, and rollback (disable policy <name>; note what deletion may already have processed and cannot be rolled back). Log time (log_time_entry).
