...
4. Reassignments and connector policies are client-visible and can stop automations — get client approval (send_approval) with the affected flows and connectors listed. Never disable a flow or change ownership on assumption; a "dead-looking" flow may run monthly.
...
6. Verify with evidence: reassigned flows show the new owner and still run (check next run/history); disabled flows are intentionally off; the DLP connector policy shows applied and existing critical flows still function. Document what/why/when/rollback — post a plain-text note (add_ticket_note): flows inventoried, orphans reassigned/co-owned or disabled (with the business process each serves), connector policy set, approver, date, and rollback (restore prior owner; re-enable a flow; remove/relax the DLP policy). Log time (log_time_entry).
