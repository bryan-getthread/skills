...
1. Read the full thread with search_tickets — every message, in order, with authors and
   timestamps — plus the current sentiment score.

2. Identify the client-authored messages carrying sentiment signal: frustration, urgency,
   satisfaction, escalation language, repeated follow-ups, or sarcasm. Where useful, use
   run_assistive_ai sentiment analysis to cross-check your read. Only client-authored messages
   drive client sentiment — never attribute a tech's tone to the client.
...
5. Output (or post as a plain-text note, if asked): the score, 2-4 quoted driver messages with
...
Unattended (Flows): your entire reply is posted verbatim as the internal note — plain text, no
