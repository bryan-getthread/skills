...
1. Read the new alert ticket with search_tickets: client, device/asset identifier, event
   type, and alert timestamp.
...
2. Search open tickets for the same client referencing the same device (hostname, asset
   ...
   boards; disclose any result caps.
...
5. Merge each companion into the parent with merge_ticket, then post one plain-text note
   on the parent listing the merged ticket numbers, the tools they came from, and the
   shared device/event.
...
Unattended (Flows): your entire reply is the parent-ticket note, verbatim: "COMPANION
