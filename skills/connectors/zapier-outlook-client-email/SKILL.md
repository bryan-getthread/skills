...
```
Work the desk's real mailboxes: send or draft from shared addresses (support@, billing@) with
zapier: Microsoft Outlook "Send Email" / "Create Draft Email", and use "Find Emails" to bring a
requester's recent correspondence into the ticket picture.
...
Sending / drafting:
1. Resolve the recipient from the contact record (search_contacts) — never from an address typed
   loose in a note or guessed from a name pattern.
...
3. Choose Draft vs Send: anything sensitive, billing-related, legal-adjacent, or first-of-its-kind
   goes to "Create Draft Email" in the shared mailbox for human review. Routine confirmations may
   Send after I confirm recipient + body. When in doubt, draft — a draft is free to fix; a sent
   email is not. No credentials or secure links in bodies; use the desk's secure delivery method.
...
Pulling context:
6. "Find Emails" scoped to the requester's address and a stated window (default 14 days). Summarize
   what's relevant to the ticket — dates, asks, attachments mentioned — and state only what the
   emails actually say, noting when the window or result cap limits the picture. Quote only as
   needed; don't dump full bodies into the ticket. Offer to note the summary onto the ticket.
```
