...
1. Resolve the Xero contact via "Find Contact" matched from the client record — ambiguous or missing
   → stop and ask; a standing check against the wrong contact is misinformation.
...
3. Pull the ticket's time entries (search_tickets including time entries): tech, duration, date, work
   notes, billable flag / coverage as the desk records it. Coverage ambiguous → ask; misbilling a
   covered client costs more than the invoice.
...
5. Assemble line items from actual time entries and materials, at rates the member confirms — never
   assumed; each line carries the ticket reference. Send the approval via send_approval (or the
   Teams/Slack gate) to whoever owns billing, with full detail and total. Approved → "Create Invoice"
   as a DRAFT; note the draft's number on the ticket. Not approved/timeout → nothing created; note why.
