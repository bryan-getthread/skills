...
1. Resolve the environment (liongard_environment), then liongard_launchpoint filtered by systemType "UniFi" (try "Ubiquiti"). Verify last-run success and note the dataprint age — carry "as of <timestamp>." Multi-site controllers: identify which site maps to this client before answering.
...
3. "What changed?" → liongard_detection / liongard_timeline: device adds/removals, firmware upgrades, WLAN edits around the incident window (time-adjacency, not proven cause).
4. Never expose wifi PSKs in notes — point to the documentation platform. Output: inventory/config table, source + dataprint age, flags (offline devices, firmware drift, weak WLAN security, single-AP-per-site SPOFs). Offer a plain-text add_ticket_note. Degradation: inspector absent → docs (search_itglue/search_hudu) → ticket history (search_tickets) → "verify on controller."
```
