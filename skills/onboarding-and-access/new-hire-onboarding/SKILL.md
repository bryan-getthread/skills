...
1. Gather from the ticket (and the client's intake form if one exists): full name,
   ...
   Use search_contacts and search_clients to anchor the client and manager records.
   List everything missing and ask me for it in ONE message, not piecemeal.

2. Check the start date. If it is within 48 hours, raise ticket priority
   (update_ticket, using the board's real names from list_ticket_priorities) and
   flag the dispatcher in a note so it can't sit in the queue.

3. Pull the client's onboarding SOP from search_knowledge_base / search_itglue /
   search_hudu and build the checklist in three categories: Required (every hire
   ...
   need explicit confirmation). If IT Glue/Hudu aren't enabled, fall back to
   search_knowledge_base and say external docs weren't checked.
...
6. Confirm the full plan with me including a timeline (account-ready date, hardware
   ETA). For licenses that carry cost, get sign-off first via send_approval or the
   client's documented approval channel.
