...
The runbook for the conversation MSPs keep having: a client is on LastPass, is asking about the 2022 security incidents, and wants a plan. This skill does two things — runs the migration-away as a rotation-first project, and equips the desk to discuss breach history responsibly (facts only, dates verified, no embellishment and no minimizing). It builds on password-manager-rollout for the destination platform's structure. Verify every breach claim against LastPass's own disclosures and current public reporting before repeating it to a client.

## When to use

- A client asks whether they should move off LastPass, or references the LastPass breaches
- A migration off LastPass to another password manager is being planned or executed
- Post-incident credential-rotation planning for a current/former LastPass tenant

## Steps

1. Breach-history discussion — defensive writing, facts only (per defensive-writing-standard): state what is publicly documented, attribute it, and verify dates before repeating them — do not paraphrase from memory into specifics. The publicly disclosed facts are that LastPass reported security incidents in 2022 in which an unauthorized party obtained a copy of customer vault data (some fields encrypted, some metadata such as URLs not encrypted). Do not invent breach specifics, victim counts, or causal claims; do not speculate about a particular client being affected; and do not downplay it either. If a client asks "were we affected," the honest answer is scoped to what is knowable: any vault data that existed in the affected window should be treated as potentially exposed and therefore rotated — that is the actionable posture regardless of unprovable specifics. Link to the vendor's official notices rather than reconstructing them.
...
3. Migration-away runbook — a rotation-first project, not a copy-paste:
   ...
   - Rotate everything, prioritized: this is the whole point. Every credential that lived in LastPass is treated as compromised-until-rotated — privileged/infrastructure first, then shared accounts, then reused passwords, then the rest. MFA on critical accounts verified/added during rotation. Track rotation as the real completion metric, not "items imported."
...
5. Document: the client's understanding of the posture (facts communicated, links provided), the destination structure, rotation progress by privilege tier, and decommission evidence. Create tickets (create_ticket) per migration phase and per rotation tier.

## Guardrails

- Facts only on breach history: verify dates and details against LastPass's official disclosures/current public record before stating them; attribute; never invent specifics or victim counts, never speculate a specific client was hit, never minimize.
- The exported LastPass vault file is plaintext credential material — never attach to a ticket, store transiently on a controlled host only, delete after import with evidence.
- Migration without rotation is theater — every LastPass-resident credential is compromised-until-rotated, privileged first; rotation progress is the completion metric.
- Credentials never appear in tickets, notes, chat, or this skill's output — locations, counts, and rotation status only.
- Platform selection is an account-management/vCIO decision; the desk owns the rotation obligation and the migration mechanics, not the sell.
- Console/export/rotation actions are technician steps the agent directs and records.
