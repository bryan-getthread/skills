...
4. Branch by alert family:
   - Unwanted access / suspicious login → run the impossible-travel-runbook verification ladder: VPN/egress plausibility from documentation (search_itglue), prior context (search_tickets, same user + type, 90 days), then verify with the user by a number on file — never contact details from the ticket or the possibly-compromised mailbox.
