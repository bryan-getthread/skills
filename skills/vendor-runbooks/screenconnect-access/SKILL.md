...
4. Use context sources you do have: ticket history (search_tickets, same device/instance, recent window) for recurrence, documentation (search_itglue) for the instance URL, version, and access-policy, and — if a Liongard inspector covers the endpoint — corroborating state (verify last run, note age; degrade if absent). There is no native ScreenConnect read integration, so state is inferred from ticket/docs/Liongard — give the source and freshness, and never present it as a live console read.
