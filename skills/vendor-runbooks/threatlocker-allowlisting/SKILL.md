...
Vendor runbook for ThreatLocker's default-deny (zero-trust) model. Unlike detection products, ThreatLocker's daily driver is **approval-request triage** — an application was blocked because it isn't on the allowlist, and a human must decide whether it should be. This skill is the discipline for making that decision safely, plus Learning-vs-Secured mode, Elevation Control, and Storage Control. The block is the product working as designed; the risk is approving too fast. Verify feature names and console layout against ThreatLocker's current documentation.

## When to use

- A ThreatLocker approval request (blocked application/script needing allowlisting) lands — the routine, high-volume case
- An Elevation Control request (run-as-admin) or Storage Control (USB/network-share access) request arrives
- A client is being onboarded and the Learning-vs-Secured mode transition needs handling
...
- Every approval is a security decision — never approve to clear a ticket; unestablished legitimacy means investigate or leave denied, not approve.
- Narrowest effective scope always: publisher > hash > path > folder; per-client/policy-group over global; never a path allow in a user-writable location.
- A client left in Learning mode is not being protected — flag it; only Secured mode enforces default-deny.
- Elevation and Storage requests carry a privilege/data lens on top of legitimacy — scope tight, time-box, and route policy changes to the client's authorized approver.
- The block is the product working; a flood of requests after go-live is expected — don't "fix" it by loosening broadly.
- Approvals and policy changes are technician/admin actions in the ThreatLocker console; the agent triages, recommends a scoped decision, and records it — it does not silently self-approve.
- Client-facing wording per defensive-writing-standard; degradation — without documentation access (search_itglue), the client's expected software baseline is unknown, so lean toward investigate-don't-approve and say so.
