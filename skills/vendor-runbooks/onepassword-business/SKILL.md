...
1. Scope and structure per password-manager-rollout, in 1Password terms: every user has a Private vault (invisible to admins by design — admins cannot see its contents); shared vaults by team/function granted via groups, never one giant company vault — vault access is managed by group membership. Privileged/infrastructure credentials go in a tightly-scoped admin vault. Record the structure in the client's documentation (search_itglue / search_hudu). Vault/group decisions get client sign-off.
