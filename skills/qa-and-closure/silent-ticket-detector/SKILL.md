...
2. Find candidates with search_tickets: open tickets whose most recent client-visible message is
   inbound (from the client) and older than the threshold, with no outbound client-visible reply
   after it. Include tickets flipped to a customer-responded status that nobody picked up. Split
   searches per board; disclose result caps — the silent ticket you miss is the one that churns
   the client.
...
5. For each silent ticket, add_ticket_note (internal, plain text) @mentioning the assigned tech —
   resolve via search_members — with: hours the client has been waiting, one-line gist of what
   the client asked, and "reply needed." If unassigned, flag for dispatch instead of mentioning
   nobody. @mention notes state facts, never blame language.
...
Unattended (Flows): trigger is a timer sweep, or customer-responded status older than the
