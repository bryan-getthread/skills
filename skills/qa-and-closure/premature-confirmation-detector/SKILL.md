...
1. Scope: one ticket, or a window of closed tickets via search_tickets (split per board, disclose
   caps).
...
5. For each PREMATURE ticket, on approval, perform both steps in order (never one without the
   other): (a) update_ticket back to a working or waiting-confirmation status (map via
   list_ticket_statuses); (b) add_ticket_note internal, plain text, no markdown: "Reopened -
   closed without customer confirmation. Last client message: <date, one-line gist>. Needed:
   client confirmation or a documented verbal confirmation. Please confirm with <user> and
   re-close."
...
Unattended (Flows): trigger is status change to closed / completed. Your entire reply is the
