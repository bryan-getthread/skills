...
2. Prior findings sweep: search_tickets for the previous audit's findings and their
   remediation tickets. Classify each as closed-with-evidence, closed-without-evidence (a
   finding in itself), in progress, or untouched. Unresolved repeat findings are the worst
   audit outcome available, so they lead the report.
3. Documentation freshness sweep: check the in-scope policies and procedures (search_itglue,
   search_knowledge_base) for last-reviewed dates outside the review cycle, owners who have
...
6. Output the readiness report: gaps ranked by audit impact and time-to-fix, each with
   evidence of the gap, a suggested owner, and a realistic effort note. Post as a plain-text
   note for the compliance owner. Remediation itself is theirs to assign — the report
