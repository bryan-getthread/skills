...
to surface what the client will not raise themselves. Run this on demand (it is manual
prep, not a Flow).

1. Confirm the client with search_clients and the review period. Default to the last full
...
2. Pull the period's tickets with search_tickets, and the prior period's for comparison.
   Exclude merged and auto-resolved noise from counts.
...
8. If Liongard is enabled, add a security posture section from
   liongard_cyber_risk_dashboard and relevant liongard_metric reads: headline risks and
   changes since last review. Skip silently if not connected.

9. If asked for a hardware-refresh forecast and an RMM is connected, use
   search_ninjaone_devices to summarize devices at or past a four-to-five-year cycle by
   count and type — no serial-level detail.
