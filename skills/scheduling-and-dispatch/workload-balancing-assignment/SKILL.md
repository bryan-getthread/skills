...
4. Set the owner and record the rationale in a plain-text note: the winning score, the
   runner-up, and the formula inputs ("Assigned <tech>: score 7 (base 10, 2 open weighted
   2, 1 scheduled block). Runner-up <tech>: 5.").
...
narration, no questions. Assign only on a single clear top scorer after exclusions. Tie or
empty pool → no assignment and post "Workload balancer: no unambiguous candidate (reason).
Left unassigned for dispatcher." Never reassign a ticket that already has an owner (do
nothing, post nothing). Never change status or priority.
